...
public class UnitGenerator : MonoBehaviour {
	...
	public void generateUnit(){
		GameObject newUnit;
		GameObject unitToGen;

		for (int i = 0; i < UnitDataController.maxUnitNum; i++){
			unitToGen = UnitDataController.GetInstance ().getUnitToGenerate ();
			if (!unitToGen)
				break;

			newUnit = Instantiate (unitToGen);
			newUnit.transform.position = transform.GetChild(i).transform.position;
